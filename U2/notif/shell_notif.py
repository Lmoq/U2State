...
class NotifLog( metaclass = Meta ):
    ...
    @classmethod
    def set_title( cls, titles: dict[ str, str ] = None ):
        # Set title where tuple[0] is displayed name and tuple[1] is the attribute reference as value
        cls.titles = titles
    ...
    @classmethod
    def post_notif( cls, update_title = True ):
        ...
        cm = f"cmd notification post -S inbox { lines } -t { repr(cls.title_str) } tag text"
        _exec(cm)


    @classmethod
    def notif( cls, log ):
        # Adds line to shell notification list and display it right away
        cls.add_line( log )

        cls.update_title( render = False )
        cls.post_notif( update_title = False )
...
def _set_process_caller( system_type ):
    ...
    def bash_preset( args ):
        subprocess.run( f'echo "{args} &> /dev/null" > ~/pipes/adbpipe', shell=True, stdout=subprocess.DEVNULL )
